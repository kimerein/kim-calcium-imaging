function [h,yOffsets]=plotComponentTraces(data,baselineWindowIndex,timeWindowIndex,nBins,zeroZero,h,yOffsets,c)
...
for i=1:length(data)
    ...
    histgrams.x{i}=1:size(changes,2);
    histgrams.y{i}=nanmean(changes,1);
end
med=median(allbases);
allchanges=allchanges(allbases>=med,:);
histgrams.x{end}=1:size(allchanges,2);
histgrams.y{end}=nanmean(allchanges);
