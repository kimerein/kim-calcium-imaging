function plot_dFoverF_vs_opto_AsFunctionOfBehavior(C_df,acq_obj,nameOptoCommand,Yr,A_or,C_or,b2,f2,Cn,options,nameBehaviorCommand)
...
plot_components_GUI_withopto_andBeh(Yr,A_or,C_or,b2,f2,Cn,options,opto_stim_resample,beh_resample);

% Get average opto-triggered responses
[optoTriggeredResponses,avOpto,trialByTrialBeh]=getOptoResponseAcrossCells(opto_stim_resample,C_df,beh_resample);
...
function [optoTriggeredResponses,avOpto,trialByTrialBeh]=getOptoResponseAcrossCells(opto_stim,C_df,beh)
    ...
    function plot_component(i)
       if i <= nr
        ...
        if i <= nr
            ...
            leg = legend('Raw trace (filtered)','Inferred','Opto Stim','Angular Velocity');
            set(leg,'FontSize',14,'FontWeight','bold');
            drawnow;
            hold off;
            if make_gif
