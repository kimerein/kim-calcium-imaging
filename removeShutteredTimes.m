    ...
    if isOpto==1
        if insertOptoDelta==1
            if resp.shutter_only~=1
                ...
                if any(backup_otherData(times>=shutterOffTimes(i,1) & times<=shutterOffTimes(i,2))>opto_on_thresh)
                ...
                if all(backup_otherData(times>=shutterOffTimes(i,1) & times<=shutterOffTimes(i,2))<opto_on_thresh)
                    timepointAfterShutter=find(times>shutterOffTimes(i,2),1,'first');
                    %             % Make first time point after shuttered window high
